...
public class HealthyRecipe extends JFrame {
    ...
    public HealthyRecipe() {
        super("14.26 Cooking with Healthier Ingredients");
        out.setEditable(false);
        JButton b = new JButton("Suggest Replacements");
        b.addActionListener(e -> suggest());
    ...
    void suggest() {
        String s = recipe.getText(), r = s.toLowerCase();
        StringBuilder x = new StringBuilder();
        for (var e: subs.entrySet()) if (r.contains(e.getKey())) x.append(e.getKey()).append(" -> ").append(e.getValue()).append("\n");
        x.append("\nAlways consult your physician before making significant changes to your diet.");
        out.setText(x.toString());
    }
    public static void main(String[] z) {
        new HealthyRecipe().setVisible(true);
    }
}
