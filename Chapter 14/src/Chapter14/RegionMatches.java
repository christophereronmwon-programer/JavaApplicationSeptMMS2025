...
public class RegionMatches extends JFrame {
    ...
    public RegionMatches() {
        super("14.4 Comparing Portions of Strings");
        setLayout(new FlowLayout());
        add(new JLabel("String 1:"));
        add(s1);
        add(new JLabel("String 2:"));
        add(s2);
        add(new JLabel("Start index:"));
        add(start);
        ...
        b.addActionListener(e -> {
            try {
                int st = Integer.parseInt(start.getText()), n = Integer.parseInt(count.getText()); boolean ok = s1.getText().regionMatches(true, st, s2.getText(), st, n); out.setText(ok ? "Equal": "Not equal");
            } catch (Exception ex) {
    ...
    public static void main(String[] z) {
        new RegionMatches().setVisible(true);
    }
}
